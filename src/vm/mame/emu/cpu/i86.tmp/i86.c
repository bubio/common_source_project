...
//{
//	return i386_dasm_one(buffer, cpustate->pc, oprom, 1);
//}
...
static CPU_EXECUTE( i80186 )
{
	...
	if(now_debugging) {
		if(force_suspend) {
			...
			UINT32 eip = cpustate->pc - cpustate->base[CS];
			for(int i = 0; i < MAX_BREAK_POINTS; i++) {
				if(break_point.table[i].status == 1 && break_point.table[i].seg == cpustate->sregs[CS] && break_point.table[i].ofs == eip) {
					break_point.hit = i + 1;
					now_suspended = true;
					break;
				}
	...
	add_cpu_trace(cpustate->pc, cpustate->sregs[CS], cpustate->pc - cpustate->base[CS], 0);
	m_prev_cs = cpustate->sregs[CS];
	m_prev_eip = cpustate->pc - cpustate->base[CS];
#endif
...
static CPU_EXECUTE( v30 )
{
	...
	add_cpu_trace(cpustate->pc, cpustate->sregs[CS], cpustate->pc - cpustate->base[CS], (cpustate->MF == 0));
	m_prev_cs = cpustate->sregs[CS];
	m_prev_eip = cpustate->pc - cpustate->base[CS];
#endif
